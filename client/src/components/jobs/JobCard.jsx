...
const JobCard = ({ job, userProfile, existingStatus }) => {
  ...
  // --- 1. SYNCED ELIGIBILITY CHECKS (FIXED) ---
  const isVerified = userProfile?.verification?.isVerified || false;
  const userScore = userProfile?.trustScore || 0; // Backend Score

  // Check Locks
  const isVerifyLocked = job.verifiedOnly && !isVerified;
  const isScoreLocked = job.minTrustScore > 0 && userScore < job.minTrustScore;
  
  const isLocked = isVerifyLocked || isScoreLocked; 
  // ---------------------------------------------
  ...
  const renderStatusButton = () => {
      ...
      if (isLocked) {
          return (
            <button disabled className="w-full py-2.5 rounded-lg text-sm font-bold bg-gray-800 text-gray-500 flex justify-center items-center gap-2 cursor-not-allowed border border-gray-700 opacity-70">
                <Lock size={16}/> 
                {isVerifyLocked ? "Verification Required" : "Low Trust Score"}
            </button>
          );
      }
  ...
  return (
    // CARD CONTAINER
    <div className={`bg-card/80 backdrop-blur-sm border p-5 rounded-xl transition-all duration-300 flex flex-col justify-between group h-full relative overflow-hidden ${isLocked ? 'border-red-500/20 opacity-90' : 'border-white/5 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/10 hover:border-primary/40'}`}>
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-4 pr-6">
            <div className="flex items-center gap-3">
                {/* LOGO */}
                <div className="w-12 h-12 bg-gray-800 rounded-xl flex items-center justify-center text-lg font-bold text-white overflow-hidden relative shadow-inner border border-white/5">
                    {job.company.logo ? (
                ...
                <div>
                    <h3 className="font-bold text-white group-hover:text-primary transition-colors duration-300 flex items-center gap-1.5 text-base">
                        {job.title}
                        {isCompanyVerified && <CheckCircle size={14} className="text-blue-500" fill="currentColor"/>}
                    </h3>
            ...
            {job.type === 'Remote' && (
                <span className="text-[10px] bg-purple-500/10 text-purple-400 px-2.5 py-1 rounded-full border border-purple-500/20 font-medium">Remote</span>
            )}
        </div>

        {/* INFO CHIPS */}
        <div className="flex flex-wrap gap-2 text-xs text-gray-400 mb-5">
            <span className="flex items-center gap-1.5 bg-gray-800/50 px-2 py-1 rounded border border-white/5">
        ...
        {/* --- TRUST SCORE BAR (IMPROVED UI) --- */}
        {(job.minTrustScore > 0 || job.verifiedOnly) && (
            <div className={`mb-5 p-2.5 rounded-lg border flex items-center gap-3 shadow-inner ${isLocked ? "bg-red-500/5 border-red-500/20" : "bg-gray-900/80 border-gray-800"}`}>
                ...
                <div className="text-[10px] text-gray-400 leading-tight w-full">
                    {job.verifiedOnly && (
                        <span className={`block mb-0.5 ${!isVerified ? "text-red-400 font-bold" : "text-green-400"}`}>
                           {isVerified ? "✓ Verification Passed" : "⚠ Verification Required"}
                        </span>
                    )}
                    {job.minTrustScore > 0 && (
                        <div className="flex justify-between w-full">
                           <span>Req Score: <span className="text-white">{job.minTrustScore}</span></span>
                           <span className={userScore < job.minTrustScore ? "text-red-400 font-bold" : "text-green-400 font-bold"}>
                               (Yours: {userScore})
                           </span>
                        </div>
                    )}
